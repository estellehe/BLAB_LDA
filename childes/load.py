...
dic = Dictionary.load('childs_'+unit+'_'+str(topicn)+partype+'.model.id2word')
bow = dic.doc2bow(animal)

lda = LdaModel.load('childs_'+unit+'_'+str(topicn)+partype+'.model')
...
with open('childes_'+unit+'_'+str(topicn)+'_'+partype+topicid+'.csv', 'wb') as myfile:
...
with open('childes_'+unit+'_'+str(topicn)+partype+'.csv', 'wb') as myfile:
    wr = csv.writer(myfile, quoting = csv.QUOTE_ALL)
    for n in nlist:
        wr.writerow(n)

with open('childes_'+unit+'_'+str(topicn)+partype+'pro.csv', 'wb') as myfile:
    wr = csv.writer(myfile, quoting = csv.QUOTE_ALL)
    for n in maxpro:
        wr.writerow(n)